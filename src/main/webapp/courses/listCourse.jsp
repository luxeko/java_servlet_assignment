                        ...
                        <div class="d-flex justify-content-center align-items-center">
                            <c:choose>
                                <c:when test="${sessionScope.role != null && sessionScope.role == 'admin'}">
                                    ...
                                    <a onclick="return confirm('Are you sure you want to delete <c:out value="${course.name}"></c:out>?')"
                                       href="CourseServlet?action=delete&id=<c:out value="${course.id}"></c:out>"
                                       class="btn btn-danger"><i class="bi bi-trash"></i></a>
                                </c:when>
                                <c:otherwise>
                                    <a href="CourseServlet?action=signIn&id=<c:out value="${course.id}"></c:out>"
                                       class="btn btn-success text-white me-2"><i class="bi bi-plus"></i> Đăng kí</a>
                                </c:otherwise>
                            </c:choose>
